...
inherit eutils multilib autotools toolchain-funcs flag-o-matic virtualx fdo-mime gnome2-utils git-r3
...
DESCRIPTION="AceStream player"
HOMEPAGE="http://acestream.org/en"
if [ "${PV%9999}" != "${PV}" ] ; then # Live ebuild
	SRC_URI=""
elif [[ "${MY_P}" == "${P}" ]]; then
	SRC_URI="http://download.videolan.org/pub/videolan/vlc/${PV}/${MY_P}.tar.xz"
else
...
IUSE="a52 aalib alsa altivec atmo avahi +avcodec
	+avformat bidi bluray cdda cddb chromaprint dbus dc1394 debug dirac
	directfb dts dvb +dvbpsi dvd elibc_glibc egl +encode faad fdk
	fluidsynth +ffmpeg flac fontconfig +gcrypt gme gnome gnutls
	growl httpd ieee1394 jack kate kde libass libcaca libnotify
...
src_unpack() {
	EGIT_REPO_URI="https://bitbucket.org/AceStream/${MY_PN}.git" \
		git-r3_src_unpack
	if [ "${PV%9999}" != "${PV}" ] ; then
		git-r3_src_unpack
	else
...
src_prepare() {
	...
	epatch "${FILESDIR}"/vlc-2.1.0-newer-rdp.patch
	epatch "${FILESDIR}"/vlc-2.1.0-libva-1.2.1-compat.patch

	# Fix up broken audio when skipping using a fixed reversed bisected commit.
	epatch "${FILESDIR}"/vlc-2.1.0-TomWij-bisected-PA-broken-underflow.patch
	...
	#AceStream
	local x _as="${WORKDIR}"/${P}
	cp -rf "${_as}"/${MY_PN}/* "${S}/"
	find "${S}" -regex '[^_]*vlc[^/]*\.\(sh\|desktop\).*$' -printf '%P\n'| \
	while read x; do \
		mv "${S}/${x}" "${S}/${x/vlc/${MY_PN}}"
	done
	mv "${S}"/po/{vlc,${MY_PN}}.pot
	EPATCH_FORCE='yes' EPATCH_SUFFIX='patch' epatch "${_as}"/patches

	# Bootstrap automake files
	find modules -type f -name Modules.am -printf '%h\n'|while read x; do
		sh ./modules/genmf ${x#*/}
	done
...
src_install() {
	...
	local x
	find "${ED}" -regex '.*/\(bin\|man1\|pkgconfig\)/.*vlc.*' | \
	while read x; do \
		mv "${x}" "${x/vlc/${MY_PN}}"
	done
}
