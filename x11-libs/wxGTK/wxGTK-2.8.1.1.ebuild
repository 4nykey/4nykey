...
RDEPEND="X? ( opengl? ( virtual/opengl )
		...
		x11-libs/libXxf86vm
		gnome? ( >=gnome-base/libgnomeprintui-2.8 gnome-base/gnome-vfs ) )
...
src_unpack() {
	...
	epatch "${FILESDIR}/${PN}-wxrc_link_fix.patch"
...
src_install() {
	...
	cp ${D}/usr/bin/wx-config ${D}/usr/bin/wx-config-${SLOT} || die "Failed to cp wx-config"
