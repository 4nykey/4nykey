...
inherit cargo desktop bash-completion-r1 xdg
if [[ -z ${PV%%*9999} ]]; then
	inherit git-r3
	...
	"
	SRC_URI="
		mirror://githubcl/jwilm/${PN}/tar.gz/${MY_PV} -> ${P}.tar.gz
...
HOMEPAGE="https://github.com/jwilm/${PN}"

LICENSE="Apache-2.0"
SLOT="0"
IUSE="bash-completion lto ncurses zsh-completion"
...
src_prepare() {
	use lto || sed -e '/lto = /s:true:false:' -i Cargo.toml
	default
}

src_install() {
	local DOCS=( {CHANGELOG,README}.md alacritty.yml )
	cargo_src_install
	einstalldocs
	domenu ${PN}.desktop
	newman ${PN}.man ${PN}.1

	use bash-completion && newbashcomp ${PN}-completions.bash ${PN}
	if use zsh-completion; then
		insinto /usr/share/zsh/site-functions
		newins ${PN}-completions.zsh _${PN}
	fi
