...
src_prepare() {
	sed -e '/examples/d; /tests/d' -i makefile
	sed -e 's:\<xsd\>:xsdcxx:g' -i doc/xsd.{1,xhtml}
	epatch \
		"${FILESDIR}/3.3.0-fix_include.patch"
}
...
cxx_gnu_optimization_options :=
	...
	cat >> build/import/cli/configuration-dynamic.make <<- EOF
cli_installed := y
	EOF
...
src_compile() {
	default
	if use doc ; then
		emake -C "${S}/doc/cxx/tree/reference" || die "generating reference docs failed"
	fi
	if use test ; then
...
src_install() {
	...
	dohtml -r -A xhtml -A pdf doc/*

	dodoc NEWS README FLOSSE doc/custom-literals.xsd
